      ...
      for (var iface in adapters) {
        if (!adapters[iface][0].internal) {
          ...
          if (adapters[iface][0].netmask) {
            ...
            iface = adapters[iface][0].netmask;

            /* Convert netmask to CIDR notation if IPv6 */
            if (validation.test(validation.test.ipv6, iface)) {
              var v6 = ip.v6.Address;
              iface = new v6(iface).subnet.substring(1);
            }

            ret.push(adapters[iface][0].address+'/'+iface);
          }
        }
      }
