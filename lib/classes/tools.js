...
class tools extends emitter {
  ...
  funcs(opts) {
    ...
    Object.keys(opts.range).forEach(function blocks(block) {
      ...
      funcs[range] = function anon(callback) {
        const report = [];


        // Capture the flags, & ranges as a command string
        let cmd_string = scope.command(opts, range);

        // Split up cmd for .spawn()
        cmd_string = cmd_string.replace(/  +/g, ' ');
        const obj = cmd_string.split(" ");
        const cmd = obj[0];
        const args = obj.slice(1);

        if (opts.verbose)
          console.log(`Running: ${cmd_string}`);
  ...
  init(defaults, opts, cb) {
    ...
    validation.init(opts, (err, result) => {
      ...
      opts.range = network.validated(opts);
