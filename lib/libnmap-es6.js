...
import {exec as proc} from 'child_process';
const v6 = ip.Address6;

const nmap = (options, fn) => {
  ...
  var tools = tools || {
    ...
    command(opts, block) {
      ...
      const ip = new v6(block);
      const ipv6 = (ip.isValid()) ? ' -6 ' : ' ';
  ...
  var network = network || {
     ...
     */
    range(opts, cidr) {
      let split = cidr.size / opts.blocksize;
      const results = [];

      split = (split > 256) ? Math.round(split / 255) : split;

      cidr.forEach((ip, long, index) => {

        if (index % split === 0) {
          const obj = ip.split('.');
          const start = parseInt(obj[3]);
          const end = (parseInt(obj[3]) + split) - 1;
          let range = null;
