  ...
  var config = config || {
    ...
    init: function(defaults, opts, cb) {
      ...
      if (/discover/.test(caller.getData().functionName)) {
        opts.ports = '';
        opts.range = tools.adapters(opts);
        opts.flags = [
          '-oX -',
          '-sn',
          '--disable-arp-ping'
        ...
        funcs[range] = function block(callback) {
          ...
          var report = []
            , execute = proc(cmd, function exe(err, stdout, stderr) {
              if (err)
                return reporting.errors(opts, err, callback);
            });

          execute.stderr.on('data', function errbytes(chunk) {
            return reporting.errors(opts, chunk, callback);
          });

/*
          execute.stderr.on('end', function errbytesend() {
            if (errors.length > 0)
              return reporting.errors(opts, errors, callback);
          });
*/
