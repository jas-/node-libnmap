...
var version = 'v0.2.0'
  ...
  var validation = validation || {
    ...
    init: function(opts, cb) {
      ...
      fs.exists(opts.nmap, function (exists) {
        if (!exists)
          return cb(new Error('Nmap binary does not exist at '+opts.nmap));
      });

      if (opts.range) {
        if (!/array|object/.test(typeof(opts.range)))
          return cb(new Error(this.rangeErr));
      }
      ...
      if (opts.ports) {
        ...
        if (!/^(?:(?:^|[-,])(?:[1-9][0-9]{0,3}|[1-5][0-9]{4}|6(?:[0-4][0-9]{3}|5(?:[0-4][0-9]{2}|5(?:[0-2][0-9]|3[0-5])))))+$/.test(opts.ports))
          return cb(new Error(this.portErr));
      }
    },
    ...
    verify: function(host, cb) {
      if (this.test(this.net.hostname, host) ||
        ...
        cb(new Error('Supplied host (' + host + ') did not pass validation. ' +
          this.rangeErr));
      }
    },
  ...
  nmap.prototype.discover = function(obj, cb) {
    ...
    var opts = config.init(defaults, obj, cb);
