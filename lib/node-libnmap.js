  ...
  var methods = methods || {
    ...
    discover: function(opts){
      ...
      var adapters = setup.adapters()
        ...
        , child = false
        , blocks = []
        , neighbors = [] // globally assigned within 'discover' method
        , tmp = []
        , obj = {}
        , cmd = false
      ...
      adapters.forEach(function(details){
        ...
        // adapter properties, I want this combined with
        // the neighbors array (below) available as a returned object
        obj = details
        ...
        blocks.forEach(function(block){
          ...
          child = exec(cmd, function(error, stdout){
            ...
            res = tools.neighbors(stdout)
            if (res.length > 0) {
              tmp = neighbors.concat(res) // combine results
              neighbors = tmp
            }
            // neighbors is declared 'global' to the 'discover' method
            // and yet I am not able to use it after this?
          })
        })

        obj.neighbors = neighbors // combine all neighbors found
        return obj // return value from 'discover' method
      })
    }
  }
