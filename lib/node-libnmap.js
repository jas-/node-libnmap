...
var version = 'v0.0.8'
  ...
  var setup = setup || {
    ...
    check: function(){
      if (Number(process.version.match(/^v\d+\.(\d+)/)[1]) <= 11)
        throw new Error({Error: 'Requires node.js v0.11.* and above'})
    },
    ...
    verify: function(opts){
      if (opts.range) {
        if (!/array|object/.test(typeof(opts.range)))
          throw new Error({Error: 'Range must be an array of host(s), examples:'+
      ...
      if (opts.ports) {
        ...
        if (!/^(?:(?:^|[-,])(?:[1-9][0-9]{0,3}|[1-5][0-9]{4}|6(?:[0-4][0-9]{3}|5(?:[0-4][0-9]{2}|5(?:[0-2][0-9]|3[0-5])))))+$/.test(opts.ports))
          throw new Error({Error: 'Port(s) must match one of the following examples:'+
  ...
  if (methods[method]){
    ...
    throw new Error({Error: 'Method "'+method+'" does not exist, please see node-libnmap API'})
  }
}
