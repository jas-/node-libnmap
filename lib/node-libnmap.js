...
var version = 'v0.2.0'
  ...
  var reporting = reporting || {
     ...
     * @returns {Function}
     */
    reports: function(opts, report, reports, fn) {
      var xml = report.join('');
      ...
      if (opts.json) {
        xml2js.parseString(xml, function (err, json) {
          ...
          var obj = reporting.flatten(json);
  ...
  var tools = tools || {
      ...
      Object.keys(opts.range).forEach(function(block) {
        ...
        funcs[range] = function block(callback) {
          ...
          execute.stdout.on('end', function() {
            reporting.reports(opts, report, reports, callback);
          });
        };
      });
    ...
    command: function(opts, block) {
      var flags = opts.flags.join(' ')
        , option;
//        , v6 = ipv6.Address(block)
//        , option = (v6.isValid()) ? ' -6 ' : ' ';

      return (!opts.ports) ?
        opts.nmap + ' ' + flags + option + block :
        opts.nmap + ' ' + flags + option + ' -p' + opts.ports + ' ' + block;
    },
      ...
      cidr.forEach(function(ip, long, index) {
        if (index % split === 0) {
          var obj = ip.split('.'),
            ...
            range = obj[0] + '.' + obj[1] + '.' + obj[2] + '.' + start + '-' + end;
          results.push(range);
        }
      });
  ...
  var validation = validation || {
    ...
    init: function(opts, cb) {
      ...
      tools.exists(opts.nmap, function(err, exists) {
        if (err)
