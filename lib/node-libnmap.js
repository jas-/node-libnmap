...
var version = 'v0.0.3'
  ...
  var methods = methods || {
    ...
    scan: function(opts){
      ...
      async.map(opts.range, function(host, callback){
        opts.range = host
  ...
  var setup = setup || {
    ...
    verify: function(opts){
      ...
      if (opts.ports) {
        if (/\d+|\d+-\d+/.test(opts.ports))
          throw 'Port(s) must match one of the following examples:'+
            '22 (single) | 0-1024 (range) | 22,25,80,443 (multiple)'
      }
