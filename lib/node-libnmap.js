...
var version = 'v0.2.8'
  ...
  var config = config || {
    ...
    init: function(defaults, opts, cb) {
      ...
      if (/discover/.test(caller.getData().functionName)) {
        opts.range = tools.settings(opts);
        console.log(opts.range);
      }
  ...
  var reporting = reporting || {
     ...
     * @returns {Function}
     */
    reports: function(opts, report, fn) {
      if (opts.json) {
        var xml = report.join('')
          , parsed = null
          , error = null;
        ...
        xml2js.parseString(xml, function parseXML(err, json) {
          if(err) error = new Error(xml);

          parsed = (json) ? reporting.flatten(json) : false;
        });
      } else {
        if (!report) error = new Error("XML Report empty");

        parsed = report;
      }

      return (error) ? fn(error) : fn(null, parsed);
    },

    /**
     * @function recursive
     * @scope private
     * @abstract Recursively generates JSON object
     *
     * @param {Mixed} report Array/Object for report conversion
     *
     * @returns {Object}
     */
    recursive: function(report) {
      if((!!report) && (report.constructor === Array)) {
        if(report.length !== 1) {
          var arr = [];
          for (var i = 0; i < report.length; i++) {
            arr.push(this.recursive(report[i]));
          }
          return arr;
        } else {
          return this.recursive(report[0]);
        }
        ...
        if(Object.keys(report).length > 1) {
            ...
            if(['host', 'ports'].indexOf(key) !== -1) {
              if(report[key].length > 1) {
                ...
                for (var h = 0; h < report[key].length; h++) {
                  preventArr.push(this.recursive(report[key][h]));
                }
                obj[key] = preventArr;
              } else {
                obj[key] = [this.recursive(report[key])];
              }
            } else {
              obj[key] = this.recursive(report[key]);
            }
          ...
          return this.recursive(report[_key]);
        }
      }
     ...
     * @function flatten
     * @scope private
     * @abstract Calls recursie JSON object creation
     *
     * @param {Mixed} report Array/Object for report conversion
     *
     * @returns {Object}
  ...
  var tools = tools || {
    ...
    settings: function(obj) {
      ...
      for (var iface in adapters) {
        if (!adapters[iface][0].subnet) {
          throw new Error('Error obtaining interface properties, upgrade node');
        }

        if (!adapters[iface][0].internal) {
          ret.push(adapters[iface][0].address+'/'+adapters[iface][0].subnet);
        }
      }
    ...
    funcs: function(opts) {
      ...
      Object.keys(opts.range).forEach(function blocks(block) {
        ...
        funcs[range] = function block(callback) {
          ...
          execute.stderr.on('end', function errbytesend() {
            if (errors.length > 0)
              reporting.reports(opts, errors, callback);
          });
          ...
          execute.stdout.on('end', function bytesend() {
            if (report.length > 0)
              reporting.reports(opts, report, callback);
          });
        };
      });
      ...
      cidr.forEach(function(ip, long, index) {
        if (index % split === 0) {
          var obj = ip.split('.')
            ...
            , range = obj[0]+'.'+obj[1]+'.'+obj[2]+'.'+start+'-'+(end > 255 ? 255 : end);
