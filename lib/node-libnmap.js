...
var version = 'v0.2.1'
   ...
   * @param {Number} timeout - Number of minutes to wait for host/port response
  ...
  var reporting = reporting || {
    ...
    reports: function(opts, report, fn) {
      ...
      return (error && parsed) ? fn(error) : fn(null, parsed);
    },
  ...
  var tools = tools || {
    ...
    command: function(opts, block) {
      ...
      return (opts.ports) ?
        opts.nmap + ' ' + to + flags + ipv6 + '-p' + opts.ports + ' ' + block :
        opts.nmap + ' ' + to + flags + ipv6 + block;
    },
  ...
  var network = network || {
    ...
    range: function(opts, cidr) {
      ...
      //split = (split < 256 ? opts.blocksize : split);
